    ...
    void Start()
    {
       /*  {
...

        {
            ...
            {
                List<float> guessList = nn.FeedForward(list);
                float maxVal = -1f;
                int indexMaxVal = -1;
                for(int i = 0; i < guessList.Count; i++){
                    if(guessList[i] > maxVal){
                        maxVal = guessList[i];
                        indexMaxVal = i;
                    }
                }
                switch(indexMaxVal){
                    case 0:
                        Debug.Log("AI SAYS COLOR IS RED!");
                        break;
                    case 1:
                        Debug.Log("AI SAYS COLOR IS GREEN!");
                        break;
                    case 2:
                        Debug.Log("AI SAYS COLOR IS BLUE!");
                        break;
                    case 3:
                        Debug.Log("AI SAYS COLOR IS YELLOW!");
                        break;
                    case 4:
                        Debug.Log("AI SAYS COLOR IS PURPLE!");
                        break;
                    default:
                        break;
                }
